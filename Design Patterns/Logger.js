...
// 1. Define Log Levels
const LogLevel = { INFO: 1, DEBUG: 2, ERROR: 3 };

// 2. Concrete Handlers (Sinks)
class ConsoleHandler {
    handle(msg) { console.log(`[Console]: ${msg}`); }
}

class DBHandler {
    handle(msg) { console.log(`[Database]: Saving... "${msg}"`); }
}

class CloudHandler {
    handle(msg) { console.log(`[Cloud/AWS]: Uploading... "${msg}"`); }
}

// 3. The Logger (Subject)
class Logger {
    constructor() {
        if (Logger.instance) return Logger.instance;
        this.handlers = []; // List of observers
    ...
    log(level, message) {
        if (level >= this.level) {
            const finalMsg = `[${new Date().toISOString()}] ${message}`;
            // Notify all registered handlers
            this.handlers.forEach(h => h.handle(finalMsg));
        }
    }
}
...
logger.addHandler(new DBHandler());
logger.addHandler(new CloudHandler());

logger.log(LogLevel.INFO, "System Health OK"); 
// Result: All 3 handlers trigger!
